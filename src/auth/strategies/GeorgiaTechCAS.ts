...
export class GeorgiaTechCAS implements RegistrationStrategy {
    ...
    private async passportCallback(request: Request, username: string, profile: Profile, done: PassportDone) {
        ...
        if (username.indexOf("@") !== -1) {
            done(null, false, { message: `Usernames of the format ${username} with an email domain are insecure and therefore disallowed. Please log in with <strong>${username.split("@")[0]}</strong> instead. <a href="${config.secrets.gatech.logoutLink}" target="_blank">Click here</a> to do this.` });
            return;
        }
